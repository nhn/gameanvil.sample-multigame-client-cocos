...
import { ResultCodeConnect, ResultCodeAuth, LoginedUserInfo, Payload } from "ngt_connector";
...
export default class Connect extends cc.Component {
    ...
    public onConnect() {
        ...
        let session = TardisManager.GetInstance().GetSessionAgent();
        // 서버 접속
        // Connect(ipAdress: string, callback?: (agent: SessionAgent, resultCode: ResultCodeConnect) => void): void;
        //  ipAddress : 접속할 서버 주소. 
        //  callback : 결과를 전달 받을 콜백.(선택 사항)
        //    agent : Connect를 호출한 SessionAgent 객체.
    ...
    public onAuthenticate() {
        ...
        // Authenticate(deviceId: string, accountId: string, password: string, payload?: Payload, callback?: (agent: SessionAgent, resultCode: ResultCodeAuth, loginedUserInfoList: Array<LoginedUserInfo>, message: string, payload: Payload) => void): void;
        //  deviceId : 중복 접속을 체크하기 위해 사용.
        //  accountId : 사용자 계정. 
        //  password : 비밀번호.
        //  payload : 추가적으로 필요한 데이터가 있을 경우 사용.(선택 사항)
        //  callback : 결과를 전달 받을 콜백.(선택 사항)
        //   agent : Authenticate를 호출한 SessionAgent 객체.
    ...
    public onDisconnect() {
        ...
        // 접속 종료
        // Disconnect(reason: string, callback?: (agent: SessionAgent, resultCode: ResultCodeDisconnect, reason: string) => void, force: boolean, payload: Payload): void;
        //   reason : 접송 종료 이유. Disconnect를 호출하는 경우가 다양할 경우, 콜백에서 각각의 경우를 구분하기위해 사용.
        //   callback : 결과를 전달 받음. (선택 사항)
        //     agent : Disconnect 호출한 SessionAgent 객체.
        //     resultCode : Disconnect 결과.
        //     reason : 접속 종료 이유.
        //     force : 서버에서의 강제종료 여부. 중복접속, Kick, AdminKick 등.
        //     payload : 서버 컨텐츠에서 보내주는 추가 데이터.

        // 이 예제에서는 SessionAgent.AddOnDisconnect()로 등록한 콜백을 통해 결과를 처리하고 있습니다.
